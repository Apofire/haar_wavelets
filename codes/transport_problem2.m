...
%  Du(x,t) + Du(x,t) = f(x,t)    where D^(α) is wrt x and D^(β) is wrt t       
% with the initial condition u(0) = C          
             
%           the exact solution when f(x,t) = t + 2*x*t + x + x^2 is 
%        u(t) = x*t + x^2*t
...
for i = 1:length(t)
    for j = 1:length(x)
        f(i,j) = x(j) + 2*t(i)*x(j) + t(i) + x(j)^2;    % function values at the collocation points
 
        % true solution
        exact_sol(i,j) = x(j)*t(i) + x(j)^2*t(i); 
    end
end
