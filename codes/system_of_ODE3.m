...
a3 = -5.*E/(H + 5.*P) ;
approx_sol_y31 = a3*P + E;
a21 = 5.*(a3*P + E)/(H + 3.*P); 
approx_sol_y21 = a21*P;
a11 = (3.*a21*P)/(H + P);
approx_sol_y11 = a11*P;


D = eye(N)/(P*H');
a3 = -5.*E/(H + 5.*P);
approx_sol_y3 = a3*P + E ;
a2 = 5.*(a3*(P*H')*H + E)/(D*H + 3.*H);
approx_sol_y2 = a2*H;
a1 = 3*(a2*H)/(D*H + H);
approx_sol_y1 = a1*H;
...
exact_sol_y3 = exp(-5.*t);
exact_sol_y2 = 2.5.*(exp(-3.*t) - exp(-5.*t));
exact_sol_y1 = (15/8).*(exp(-5.*t) + exp(-t) - 2.*exp(-3.*t));
% error
error1 = zeros(length(t),3);
error1(:,1) = abs(exact_sol_y1 - approx_sol_y11);
error1(:,2) = abs(exact_sol_y2 - approx_sol_y21);
error1(:,3) = abs(exact_sol_y3 - approx_sol_y31);

% Euclidean norm of the error
error_max1 = zeros(1,3);
error_max1(1,1) = norm(error1(:,1),2); 
error_max1(1,2) = norm(error1(:,2),2);
error_max1(1,3) = norm(error1(:,3),2);
...
% Euclidean norm of the error
error_max = zeros(1,3);
error_max(1,1) = norm(error(:,1),2); 
error_max(1,2) = norm(error(:,2),2);
error_max(1,3) = norm(error(:,3),2);
...
%fig:02
 figure('color','w')
 plot(t,abs(approx_sol_y1 - approx_sol_y11))
 xlabel('$t$'); ylabel('Absolute Error');
 title('Absolute Error: $\max|y_{M1} - y_{M2}|$')
