...
% with the initial condition u(0) = 0     alpha = beta = 1     
%             f(x,t) = (x+t)cos(xt)
...
for i = 1:length(x)
    for j = 1:length(t)
        f(i,j) = (x(i)+t(j))*cos(x(i)*t(j));    % function values at the collocation points
       
        
        % true solution
        exact_sol(i,j) =  sin(x(i)*t(j)) ;
    end
end
...
F = H*f*H';
     ...
     c  = fsolve(@(c) myfun(c,H,F,D_alpha), zeros(N,N));
%     c1 = H*exact_sol*H';
...
function y = myfun(c,H,F,D_alpha)
y = H'*D_alpha'*c*H + H'*c*D_alpha*H - H'*F*H;
end
