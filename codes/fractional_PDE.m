...
% f(x,t) = ((4/3)*((x)^(3/4)*t + x*(t)^(3/4)))/gamma(3/4)       
% for alpha = beta = 1/4
...
for i = 1:length(x)
    for j = 1:length(t)
        f(i,j) = ((4/3)*((x(i))^(3/4)*t(j) + x(i)*(t(j))^(3/4)))/gamma(3/4);    % function values at the collocation points
        
        % true solution
        exact_sol(i,j) = x(i)*t(j);
    end
end
...
y = H'*D_alpha'*c*H + H'*c*D_beta*H - H'*c1*H;
end
